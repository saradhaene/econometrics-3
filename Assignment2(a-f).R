...
# data
main <- function() {
  file_path <- "data_assignment2.csv"
...
granger_test_ir <- function(data){
  ...
  # Unrestricted VAR
  prep_u <- prepare_VAR_data(data, vars, p, intercept)
  est_u  <- estimate_VAR(prep_u$Y, prep_u$X)
  ...
  # Restricted VAR 
  vars_r <- c("gdp_growth","cpi_growth")
  
  prep_r <- prepare_VAR_data(data, vars_r, p, intercept)
  est_r  <- estimate_VAR(prep_r$Y, prep_r$X)
  ...
  F_stat <- ((RSS_r - RSS_u) / q) / (RSS_u / (T_eff - k_u))
  p_val <- 1 - pf(F_stat, q, T_eff - k_u)
